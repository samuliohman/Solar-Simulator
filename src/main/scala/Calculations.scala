object Calculations {
  ...
  //Calculate the average change in position(dx) and velocity(dv) using Runge Kutta 4th order
  ...
  def calculateAccelerationRK4(bodies: Vector[Body], dt: Double): Vector[(Vector3D, Vector3D)] = {
    val planetInfo = bodies.map(p => (State(p.location, p.velocity), p.mass)) //returns Vector[(State(location, velocity), mass)] = Vector[(State(x0, v0), mass)]
    val k1 = evaluate(planetInfo, 0, Vector.fill(bodies.length)(Derivative(Vector3D(0, 0, 0), Vector3D(0, 0, 0))))
    val k2 = evaluate(planetInfo, dt / 2, k1)
    val k3 = evaluate(planetInfo, dt / 2, k2)
    val k4 = evaluate(planetInfo, dt, k3)

    val dxdt = k1.indices.map(i => (k1(i).dx + ((k2(i).dx + k3(i).dx) * 2.0) + k4(i).dx) * (1.0 / 6))
    val dvdt = k1.indices.map(i => (k1(i).dv + ((k2(i).dv + k3(i).dv) * 2.0) + k4(i).dv) * (1.0 / 6))
    (dxdt zip dvdt).toVector
  }

  def evaluate(planets: Vector[(State, Double)], t: Double, ld: Vector[Derivative]): Vector[Derivative] = {
    ...
    states.zip(planets).map { case (state, planet) => Derivative(
      state.v,
      acceleration((state.pos, planet._2), planets.diff(Vector(planet)).map(p => (p._1.pos, p._2))))
    }
  }

  def acceleration(thisPlanet: (Vector3D, Double), otherPlanets: Vector[(Vector3D, Double)]): Vector3D = {
